...
const successPresent = ref(false);
const successMessage = ref("");

watch(csvInput, () => {
  errorPresent.value = false;
  errorMessage.value = "";
  successPresent.value = false;
...
const submitInput = async () => {
  ...
  if (validInput) {
    ...
    if (res) {
      successPresent.value = true;
      successMessage.value =
        "Word(s) successfully saved. Refresh the page to see new word(s).";
    } else {
      errorPresent.value = true;
      errorMessage.value = "Unable to save word(s)";
    }
  } else {
    errorPresent.value = true;
    errorMessage.value = "Invalid input, ensure format is correct";
  }
};
</script>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <h5 v-if="successMessage">{{ successMessage }}</h5>
        ...
        <div class="modal-footer">
          <button type="button" @click="submitInput" class="btn btn-primary">
            Save
          </button>
        </div>
      </div>
